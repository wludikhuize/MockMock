...
public class Parser {
    ...
    public Settings parseOptions(String[] args, Settings settings) {
        ...
        options.addOption("w", true, "The web api port number to use. Default is 8283.");
        ...
        try {
            ...
            parseWebApiPortOption(cmd, settings);
    ...
    protected void parseWebApiPortOption(CommandLine cmd, Settings settings) {
        if (cmd.hasOption("w")) {
            try {
                settings.setWebApiPort(Integer.valueOf(cmd.getOptionValue("w")));
            } catch (NumberFormatException e) {
                System.err.println("Invalid WebApi port given, using default " + settings.getWebApiPort());
            }
        }
    }
    ...
    protected void parseMailQueueSizeOption(CommandLine cmd, Settings settings) {
        if (cmd.hasOption("m")) {
            try {
                ...
                System.err
                        .println("Invalid max mail queue size given, using default " + settings.getMaxMailQueueSize());
            }
        }
    }
